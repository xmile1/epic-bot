...
const attachments = require('./help.json');
...
class xBot extends Bot {
  ...
  onMessage(message) {
    if (this.isChatMessage(message) && this.isChannelConversation(message) && !this.isFromxBot(message)) {
      this.isMentioningAMeetingRoom(message).then((roomDetails) => {
        if (roomDetails) {
          const description = {
            attachments: [
              {
                title: `${roomDetails.roomName} Location Details`,
                text: `${roomDetails.roomName} is located at ${roomDetails.description}`,
                color: '#7CD197',
              }
            ]
          };
          this.replyWithDescription(message, description);
        }
      });
    }
  ...
  replyWhereIs(message, userToFind, selector) {
    ...
    this.db.get(`SELECT * FROM ${whereToSearch} WHERE ${constraint} = '${userToFind}'`, (err, records) => {
      ...
      if (records) {
        let description;
        const imgUrl = records.imageUrl ? `${records.imageUrl.replace(/^<|>$/, '')}` : '';
        switch (selector) {
        case 'room':
          description = {
            attachments: [
